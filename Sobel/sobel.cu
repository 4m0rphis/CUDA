...
__global__ void sobel(unsigned char *g, unsigned char *s, std::size_t cols, std::size_t rows)
{
  ...
  {
    auto h = g[(j - 1) * cols + i - 1] - g[(j - 1) * cols + i + 1] + 2 * g[(j)*cols + i - 1] - 2 * g[(j)*cols + i + 1] + g[(j + 1) * cols + i - 1] - g[(j + 1) * cols + i + 1];

    auto v = g[(j - 1) * cols + i - 1] - g[(j + 1) * cols + i - 1] + 2 * g[(j - 1) * cols + i] - 2 * g[(j + 1) * cols + i] + g[(j - 1) * cols + i + 1] - g[(j + 1) * cols + i + 1];
...
__global__ void sobel_shared(unsigned char *g, unsigned char *s, std::size_t cols, std::size_t rows)
{
  ...
  {
    auto h = sh[(lj - 1) * w + li - 1] - sh[(lj - 1) * w + li + 1] + 2 * sh[(lj)*w + li - 1] - 2 * sh[(lj)*w + li + 1] + sh[(lj + 1) * w + li - 1] - sh[(lj + 1) * w + li + 1];

    auto v = sh[(lj - 1) * w + li - 1] - sh[(lj + 1) * w + li - 1] + 2 * sh[(lj - 1) * w + li] - 2 * sh[(lj + 1) * w + li] + sh[(lj - 1) * w + li + 1] - sh[(lj + 1) * w + li + 1];
...
__global__ void grayscale_sobel_shared(unsigned char *rgb, unsigned char *s, std::size_t cols, std::size_t rows)
{
  ...
  if (i < cols - 1 && j < rows - 1 && li > 0 && li < (w - 1) && lj > 0 && lj < (h - 1))
  {
    auto hr = sh[(lj - 1) * w + li - 1] - sh[(lj - 1) * w + li + 1] + 2 * sh[(lj)*w + li - 1] - 2 * sh[(lj)*w + li + 1] + sh[(lj + 1) * w + li - 1] - sh[(lj + 1) * w + li + 1];

    auto vr = sh[(lj - 1) * w + li - 1] - sh[(lj + 1) * w + li - 1] + 2 * sh[(lj - 1) * w + li] - 2 * sh[(lj + 1) * w + li] + sh[(lj - 1) * w + li + 1] - sh[(lj + 1) * w + li + 1];
...
int main()
{
  ...
  cv::Mat m_in = cv::imread("in.jpg", cv::IMREAD_COLOR);
  // auto rgb = m_in.data;
  auto rows = m_in.rows;
  auto cols = m_in.cols;
  ...
  checkForCudaErr(cudaMallocHost(&g, rows * cols));
  ...
  checkForCudaErr(cudaMallocHost(&rgb, 3 * rows * cols));
  ...
  checkForCudaErr(cudaMalloc(&rgb_d, 3 * rows * cols));
  checkForCudaErr(cudaMalloc(&g_d, rows * cols));
  checkForCudaErr(cudaMalloc(&s_d, rows * cols));

  checkForCudaErr(cudaMemcpy(rgb_d, rgb, 3 * rows * cols, cudaMemcpyHostToDevice));
  ...
  */
  ...
  checkForCudaErr(cudaMemcpy(g, s_d, rows * cols, cudaMemcpyDeviceToHost));
  ...
  checkForCudaErr(cudaFree(rgb_d));
  checkForCudaErr(cudaFree(g_d));
  checkForCudaErr(cudaFree(s_d));

  checkForCudaErr(cudaFreeHost(g));
  checkForCudaErr(cudaFreeHost(rgb));
