...
void sobel(cv::Mat &in, cv::Mat &out) {
  ...
  for(int j = 1 ; j < height - 1 ; ++j) {
    for(int i = 1 ; i < width - 1 ; ++i) {
      int h =     in.at<uchar>(j - 1, i - 1) -     in.at<uchar>(j - 1, i + 1)
  + 2 * in.at<uchar>(j    , i - 1) - 2 * in.at<uchar>(j    , i + 1)
  +     in.at<uchar>(j + 1, i - 1) -     in.at<uchar>(j + 1, i + 1);

      int v =     in.at<uchar>(j - 1, i - 1) -     in.at<uchar>(j + 1, i - 1)
  + 2 * in.at<uchar>(j - 1, i    ) - 2 * in.at<uchar>(j + 1, i    )
  +     in.at<uchar>(j - 1, i + 1) -     in.at<uchar>(j + 1, i + 1);
